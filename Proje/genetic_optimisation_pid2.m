...
% Grafik: Tüm step yanıtları jenerasyona göre renkli
figure;
hold on;
legend_entries = cell(max_gen, 1); % Legend için girişler
for gen = 1:max_gen
    for i = 1:pop_size
        data = all_responses{gen, i};
        plot(data.time, data.response, 'Color', colors(gen, :)); % Jenerasyon rengi
    end
    legend_entries{gen} = sprintf('Jenerasyon %d', gen); % Legend girişi
end
legend(legend_entries, 'Location', 'BestOutside'); % Legend ekle
title('Tüm Jenerasyonlardaki Step Yanıtları (Renkli)');
xlabel('Zaman (s)');
ylabel('Çıkış');
grid on;

% Grafik: Optimize edilmiş PID kontrolcünün step yanıtı ve motorun açık döngü step yanıtı
figure;
hold on;
...
% Motorun açık döngü step yanıtı
...
legend;
title('Optimize Edilmiş PID ve Motor Step Yanıtı');
xlabel('Zaman (s)');
ylabel('Çıkış');
grid on;
